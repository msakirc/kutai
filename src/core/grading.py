...
async def drain_ungraded_tasks() -> int:
    """Grade all ungraded tasks.

    Called from on_model_swap() and idle path. Fatih Hoca picks the
    best model for each grading call; each task's exclude_models list
    (generating model + previously-failed graders) is forwarded via
    grade_task().
