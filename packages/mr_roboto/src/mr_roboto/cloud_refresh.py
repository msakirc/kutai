...
Periodic mr_roboto handler — fired every 6h by the ``cloud_refresh`` internal
...
system. fatih_hoca itself does NOT import mr_roboto or KDV — those wirings
...
logger = get_logger("mr_roboto.cloud_refresh")
...
async def run(task: dict) -> dict[str, Any]:
    ...
    Returns the refresh summary. Exceptions propagate to the mr_roboto
