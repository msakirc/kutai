...
async def test_auto_commit_runs_git_commit_on_success():
    ...
    with patch("mr_roboto.git_commit.ensure_git_repo", new_callable=AsyncMock) as mock_ensure, \
         patch("mr_roboto.git_commit.git_commit", new_callable=AsyncMock) as mock_commit, \
         patch("mr_roboto.git_commit.get_mission_workspace_relative", return_value="missions/7"):
...
async def test_auto_commit_skips_on_nothing_to_commit():
    ...
    with patch("mr_roboto.git_commit.ensure_git_repo", new_callable=AsyncMock), \
         patch("mr_roboto.git_commit.git_commit", new_callable=AsyncMock) as mock_commit, \
         patch("mr_roboto.git_commit.get_mission_workspace_relative", return_value="missions/1"), \
         patch("mr_roboto.git_commit.logger") as mock_logger:
...
async def test_auto_commit_uses_mission_workspace_path_when_mission_id_present():
    ...
    with patch("mr_roboto.git_commit.ensure_git_repo", new_callable=AsyncMock) as mock_ensure2, \
         patch("mr_roboto.git_commit.git_commit", new_callable=AsyncMock) as mock_commit2, \
         patch("mr_roboto.git_commit.get_mission_workspace_relative") as mock_path2:
...
async def test_router_git_commit_default_empty_diff_is_success():
    ...
    with patch("mr_roboto.auto_commit", new_callable=AsyncMock) as mock_commit:
