...
class CoderAgent(BaseAgent):
    ...
    def get_system_prompt(self, task: dict) -> str:
        return (
            ...
            "You write clean, modular, well-named code. Functions do one thing. "
            "Files have one responsibility. Variables have explicit types where "
            ...
            "7. **Pre-submit check** — Re-read the task and verify every requested "
            "function/class/endpoint exists in your code. Double-check that every "
            "import is at the top of its file.\n"
            "8. **Commit** — When everything works, use `git_commit` with a descriptive "
            "message.\n"
            "9. **Report** — Give your `final_answer` summarizing what you built.\n"
            "\n"
            "## Critical Rules\n"
            ...
            "- Do NOT leave `TODO`, `pass`, or placeholder comments in your code. "
            "If you can't implement something, say so in `final_answer`.\n"
            "- For any configurable value (timeouts, limits, paths), set an explicit "
            "default. Use type annotations in Python. Avoid `Any` unless necessary.\n"
