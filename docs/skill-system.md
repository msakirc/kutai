...
Vector matching on rich descriptions works because: the grader writes descriptions at capture time (after seeing the full execution), and the task text at query time (title + description) is usually 10-30 words — enough for meaningful embeddings with multilingual-e5-small.
...
2. **Embedding model dependency**: Vector matching requires ChromaDB + multilingual-e5-small. If ChromaDB is unavailable, all skill matching silently returns empty. Skills are still captured to SQLite (they just won't match until ChromaDB is back).
