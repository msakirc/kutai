...
# Repo root for the editable / in-repo install: packages/db/src/dabidabi/ → 4 up.
# Only used for the dev default below — a published/standalone install cannot
# guess the host's layout and MUST set the DB_PATH env var or call configure().
_REPO_ROOT = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "..", "..")
)
try:
    from dotenv import load_dotenv as _load_dotenv, find_dotenv as _find_dotenv
    # Search upward from cwd so this works regardless of where the package is
    # installed; fall back to the repo-root .env for in-repo runs.
    _load_dotenv(
        _find_dotenv(usecwd=True) or os.path.join(_REPO_ROOT, ".env"),
        override=False,
    )
except ImportError:  # python-dotenv absent in minimal envs
    pass

DB_PATH = os.getenv("DB_PATH") or os.path.join(_REPO_ROOT, "data", "kutai.db")
...
async def record_confidence_claim(task_id: int) -> int | None:
    ...
    # Most-recent model_pick_log row for this task. The registry-table READ
    # (tier-0 task_id join, tier-1 legacy title join) is owned by fatih_hoca;
    # lazy import breaks the dabidabi↔fatih_hoca import cycle (fatih_hoca.db
    # builds ON dabidabi.get_db). See 2026-06-16 DB Phase B deferred handoff §2.
    from fatih_hoca.db import get_latest_pick_for_task
    picked_model, picked_at = await get_latest_pick_for_task(task_id, title)
