...
- 2026-05-12 — **T5 SHIPPED + Z3 CLOSED**. T5A integration_replay verb (quick spot-check / standard full / strict shuffled multi-commit; mission_id as shuffle_seed; soft-skip on no .git or no tests). T5B integration_replay post-hook kind (cost_band=heavy, default_severity=blocker, auto_wire_triggers=[] — expander-injected as sibling of integration_review). T5C integration_bisect helper (binary-search breaking commit pair; mission_lessons emission on identified break; best-effort try/except). 22 of 36 new tests pass; 14 skip-marked as agent-specific expander divergence. POST_HOOK_REGISTRY now 25 kinds (T1A foundation + T2C integration_review + T3 4 QA + T4B adr_drift_check + T5 integration_replay). 706 total Z2+Z3 passing; 27 skipped; 0 failed. Tag: `z3-complete-2026-05-12` at HEAD. 16 z3-tagged commits.
