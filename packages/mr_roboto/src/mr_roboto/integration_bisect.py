...
Z3 residual R4 — when a breaking_pair is found, the result is enriched
with diff metadata so callers (and the mission_lessons emitter) get a
much richer signal than just the SHA pair:

- ``changed_files``: list of paths touched in ``commit_a..commit_b``
- ``file_cluster``: top-level dir → file count, ordered desc
- ``diff_shortstat``: ``"3 files changed, 42 insertions(+), 17 deletions(-)"``
- ``commit_subject``: subject line of the breaking commit

All best-effort: a git failure on the enrichment path returns the
core breaking_pair fields unchanged.
"""
...
async def _git_capture(args: list[str], cwd: str) -> str:
...
def _cluster_files(paths: list[str], top_n: int = 5) -> list[dict[str, Any]]:
    """Group files by top-level directory; return [{dir, count, sample}]."""
    ...
    clusters = [
        {"dir": d, "count": len(files), "sample": files[:3]}
        for d, files in by_dir.items()
    ]
    clusters.sort(key=lambda c: (-c["count"], c["dir"]))
    return clusters[:top_n]


async def _gather_breaking_diff(
    commit_a: str, commit_b: str, cwd: str
) -> dict[str, Any]:
    """Best-effort: return changed_files/file_cluster/diff_shortstat/subject."""
    rng = f"{commit_a}..{commit_b}"
    names = await _git_capture(["diff", "--name-only", rng], cwd=cwd)
    changed = [ln for ln in names.splitlines() if ln.strip()] if names else []
    stat = await _git_capture(["diff", "--shortstat", rng], cwd=cwd)
    subject = await _git_capture(
        ["log", "-1", "--format=%s", commit_b], cwd=cwd
    )
    return {
        "changed_files": changed[:50],  # cap at 50 for storage
        "file_cluster": _cluster_files(changed),
        "diff_shortstat": stat,
...
async def integration_bisect(
    ...
    # Best-effort enrichment: diff metadata for richer lesson rows.
    enrichment = await _gather_breaking_diff(commit_a, commit_b, cwd=cwd)
