...
Companion to ``test_grade_verify_authority.py`` (spawn side). When a step's
deterministic ``verify_*_shape`` / ``verify_adr_register`` check passed, the
grade continuation is tagged ``shape_verify_passed=True`` and the grade STILL
runs. This file pins the resume-handler contract:
  ...
  * a grade FAIL whose ONLY failing axis is COMPLETE is overridden to PASS
    ...
    completeness was already proven deterministically;
  ...
  * a grader that can't produce a parseable verdict, or a grade child that dies
    on infra/no-candidates, falls back to auto-PASS ONLY when the shape verifier
    already proved completeness — outage-safety parity with the old
    skip-the-grade fix (a shape-valid producer is never punished for grader
    unavailability).
"""
