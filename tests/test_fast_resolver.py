...
async def test_try_resolve_weather_query():
    ...
    with patch("src.tools.fast_resolver._find_best_match", new_callable=AsyncMock) as mock_match, \
         patch("src.tools.fast_resolver._call_best_api", new_callable=AsyncMock) as mock_call:
...
async def test_enrich_context_works_when_score_above_layer0_threshold():
    ...
    with patch("src.tools.fast_resolver._find_best_match", new_callable=AsyncMock, return_value=match), \
         patch("src.tools.fast_resolver._call_best_api", new_callable=AsyncMock, return_value={"temp": "22C"}), \
         patch("src.tools.fast_resolver._format_response", return_value="22C in Istanbul"), \
...
def test_format_weather_response():
    from src.tools.fast_resolver import _format_response
