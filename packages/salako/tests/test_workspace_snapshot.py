...
async def test_snapshot_returns_dict_with_hashes_and_sha():
    ...
    with patch("salako.workspace_snapshot.compute_workspace_hashes") as mock_hashes, \
         patch("salako.workspace_snapshot.get_commit_sha", new_callable=AsyncMock) as mock_sha, \
         patch("salako.workspace_snapshot.get_current_branch", new_callable=AsyncMock) as mock_branch, \
         patch("salako.workspace_snapshot.save_workspace_snapshot", new_callable=AsyncMock) as mock_save:
...
async def test_snapshot_returns_none_on_exception():
    ...
    with patch("salako.workspace_snapshot.compute_workspace_hashes",
