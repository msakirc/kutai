...
def _ensure_schema(conn: sqlite3.Connection) -> None:
    """Idempotent CREATE — delegates to the shared sync schema executor
    in fatih_hoca.schema (single source of truth: REGISTRY_DDL +
    REGISTRY_ALTERS). Lets this module work in tests against a fresh
    tmp DB without forcing the test to call init_db() (async). The
    shared DDL gives registry_events the FULL shape (incl mission_id/
    task_id/verb/reversibility) — an intended improvement over the
    older inline copy that lacked the action-scope audit columns."""
