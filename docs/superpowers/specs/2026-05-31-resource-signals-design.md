...
- **Local pressure ≈ S9 only today.** Local model → no entry in `snapshot.cloud` → empty matrix → S1-S7, S10, S11 return 0; S12 (pool-balance) is **free-cloud-only** (`if not is_free: return 0.0`) so it's 0 for local too; only S9 fires. New signals are the first real local-sensing expansion. ✓
- **Signal census (corrected 2026-06-09 — `s12_pool_balance` landed 2026-06-04, commit `b6b627bb`, during the planning gap).** Live signals today: **S1-S7, S9-S12 = 11** (no S8); modifiers M1-M3. `OTHER_BUCKET = ("S1","S7","S9","S10","S11","S12")`, `POSITIVE_ARM_SIGNALS = ("S9","S12")`. **S12 is taken** — the new desktop signals are **S13 (presence)** and **S14 (contention)**; M4 is the 4th modifier. ✓
...
For a local model (no entry in `snapshot.cloud`) → empty `RateLimitMatrix` → **S1-S7, S10, S11 all return 0.0**, and **S12 pool-balance returns 0** (free-cloud-only). Local-pool pressure today **is S9 alone**. The new desktop signals (**S13/S14**) are the first real expansion of local sensing; they slot into `combine.py`'s `OTHER_BUCKET` alongside S9 (weight 1.0, worst-of-negatives), and are negative-only so they stay OUT of `POSITIVE_ARM_SIGNALS`.
...
- **S13 — user-presence** (the human-attention axis)
...
- **S14 — machine-contention** (the machine-busy axis; can fire while the user is away, e.g. an overnight render)
  Inputs (both already collected — no new sensor): RAM (`SystemState.ram_available_mb`, already gathered) + external-GPU fraction (`ExternalGPUUsage.external_vram_fraction`, already maintained by the 30s `_auto_detect_loop`). S14 reads the **cached** external-GPU value from the snapshot — it does NOT call `detect_external_gpu_usage` itself (pynvml ~10-20ms too costly per admission).
...
**Net new sensing: only user-presence (S13).** RAM + external-GPU already collected; the work is plumbing them onto `SystemSnapshot` and into S14.
...
Load mode stops being a VRAM-% and becomes a **per-signal weight on S13/S14**, mirroring `M3_difficulty_weights`. New `M4_load_mode_weights(mode) -> dict[str,float]`, applied in `pressure_for` before the fold:

| Mode (Telegram button) | M4 effect on S13/S14 |
|---|---|
...
SystemSnapshot.pressure_for(model, ...):
   sig = { S1..S12,  S13=s13_presence(model, snap),  S14=s14_contention(model, snap) }   # S12 = existing free-cloud pool-balance
   weights = M3_difficulty_weights(...) ⊗ M4_load_mode_weights(load_mode)   # M4 scales S13/S14
   ... existing M1/M2 ...
   breakdown = combine_signals(sig, weights)   # S13,S14 ∈ OTHER_BUCKET, worst-of-negatives
...
- **S13/S14 unit tests** — cloud model → 0.0; local idle/away → 0.0; local + present-normal → graded negative; local + fullscreen → −10; local + external-GPU-heavy → −10; local + high-RAM → graded negative. Mirror existing `signals/` test style.
- **M4 unit tests** — Full silences (weight 0), Otomatik passthrough (1.0), Heavy/Shared amplify (>1.0), Minimal hard-vetoes.
- **Fold integration** — S13/S14 worst-wins inside OTHER_BUCKET; a −10 sentinel pegs final scalar at −1.0 through M3×M4 weight extremes `[0.5,2.0]`.
...
- **S13/S14 thresholds** (idle-away cutoff, RAM %, graded-negative slopes) are starting guesses — tune against real `kutai.jsonl` once live, like the S1/S9 thresholds were.
- **M3 ⊗ M4 composition** — both produce per-signal weight dicts; multiply them (M4 only touches S13/S14, M3 doesn't, so no collision). Confirm the multiply, not overwrite, in `pressure_for`.
