...
**No `BaseModelInfo` subclassing.** Initial spec proposed shared base + Text/Image subclasses, but Plan 1 v2's recon confirmed `ModelInfo` has 4 required fields with no defaults at the top of its dataclass (`registry.py:53-56`). Any defaulted parent would crash compile with "non-default argument follows default argument." Cleanest: **`ImageModelInfo` is an independent `@dataclass`; husam branches on `isinstance(pick.model, ImageModelInfo)` (and on `context.image_call`).** Zero touch to the 40-field `ModelInfo` hot path; `Pick.model` stays loosely typed (already `model: object` per `fatih_hoca/types.py:9`).
...
- **Telemetry parity.** The image lane mirrors the LLM lane's full envelope inside **`husam._run_image`** (NOT a dispatcher method — there is no `dispatcher.dispatch()`; the raw path is `orchestrator._dispatch → husam.run → (image) paintress.generate`): `begin_call/end_call` (in-flight registry), `_record_pick` on success+failure (`model_pick_log`, via the dispatcher's thin delegator husam borrows), `record_call_tokens` (zero tokens for image — row still writes for rollup), `record_call_cost`. All recon-confirmed generic (no LLM-only fields blocking). The local handover + `paintress.generate` are both wrapped in **`heartbeat.keepalive()`** so the 30-60s+ unload+poll+start window never trips the 300s watchdog. (Plan-1 detail: see `docs/superpowers/plans/2026-05-25-image-gen-plan1-cloud-spine.md` Task 11.)
...
- **`/image <prompt>`**: `beckman.enqueue` an image spec (no direct dispatcher call — `feedback_singular_dispatcher_caller`); **the photo is delivered via a durable CPS `on_complete` continuation (`image_delivery.resume`), NOT `await_inline`** (await_inline is being retired and cannot drive cross-cycle retries). The resume handler reaches the bot via the module singleton and gets `chat_id` from `cont_state`. No dedicated rate-limit — beckman + kdv already pace the queue (single-user system); optional deep-queue warning.
