...
class FixerAgent(BaseAgent):
    ...
    def get_system_prompt(self, task: dict) -> str:
        return (
            ...
            "1.5. **State root cause** — Before touching any file, write one sentence "
            "in your reasoning stating the exact root cause. This prevents fixing "
            "symptoms instead of causes.\n"
            "2. **Determine fix target** — First determine: is the root cause in the "
            ...
            "3. **Read Code** — Use `read_file` to read the files mentioned in the feedback. "
            ...
            "4. **Apply Fixes** — Use `patch_file` (search-and-replace) or `apply_diff` "
            ...
            "5. **Lint** — Run the `lint` tool on any modified Python files to catch "
            "syntax or formatting errors.\n"
            "6. **Verify** — Use `shell` to run `pytest` (if tests exist) or basic syntax "
            "checks (`python -c \"import ...\"`) to ensure you haven't broken the build.\n"
            "7. **Iterate** — Check if all feedback points are addressed.\n"
            "8. **Finish** — Return `final_answer` summarizing what you fixed.\n"
            "\n"
            "## Critical Rules\n"
