    ...
    async def _create_task(self, goal_id, title, status="pending",
        ...
        cursor = await db.execute(
            """INSERT INTO tasks
               ...
               VALUES (?, ?, '', ?, ?, ?, ?)""",
            (goal_id, title, agent_type, status,
...
class TestApprovalRequestsTable(_DBTestBase):
    ...
    def test_insert_and_query(self):
        async def _test():
            db = await self.db_mod.get_db()
            await db.execute(
                """INSERT INTO approval_requests (task_id, goal_id, title, details)
...
class TestInsertTasksAtomically(_DBTestBase):
    ...
    def test_basic_insert(self):
        async def _test():
            ...
            ids = await self.db_mod.insert_tasks_atomically(tasks, goal_id)
            ...
            for tid in ids:
                ...
                row = await cursor.fetchone()
                self.assertIsNotNone(row)
                self.assertEqual(dict(row)["goal_id"], goal_id)
        run_async(_test())
    ...
    def test_empty_list(self):
        async def _test():
            goal_id = await self._create_goal()
            ids = await self.db_mod.insert_tasks_atomically([], goal_id)
...
class TestPropagateSkips(_DBTestBase):
    ...
    def test_no_skipped_deps_no_change(self):
        ...
        async def _test():
            goal_id = await self._create_goal()
            t1 = await self._create_task(goal_id, "T1", status="completed")
            t2 = await self._create_task(goal_id, "T2", depends_on=[t1])

            count = await self.db_mod.propagate_skips(goal_id)
    ...
    def test_all_deps_skipped_propagates(self):
        ...
        async def _test():
            goal_id = await self._create_goal()
            t1 = await self._create_task(goal_id, "T1", status="skipped")
            t2 = await self._create_task(goal_id, "T2", depends_on=[t1])
    ...
    def test_mixed_deps_not_skipped(self):
        ...
        async def _test():
            goal_id = await self._create_goal()
            t1 = await self._create_task(goal_id, "T1", status="completed")
            t2 = await self._create_task(goal_id, "T2", status="skipped")
            t3 = await self._create_task(goal_id, "T3", depends_on=[t1, t2])
    ...
    def test_transitive_propagation(self):
        ...
        async def _test():
            ...
            t1 = await self._create_task(goal_id, "T1", status="skipped")
            t2 = await self._create_task(goal_id, "T2", depends_on=[t1])
            t3 = await self._create_task(goal_id, "T3", depends_on=[t2])
    ...
    def test_pending_deps_block_propagation(self):
        ...
        async def _test():
            goal_id = await self._create_goal()
            t1 = await self._create_task(goal_id, "T1", status="skipped")
            t2 = await self._create_task(goal_id, "T2", status="pending")
            t3 = await self._create_task(goal_id, "T3", depends_on=[t1, t2])
    ...
    def test_no_tasks_returns_zero(self):
        """Propagation on a goal with no tasks returns 0."""
        async def _test():
            goal_id = await self._create_goal()
            count = await self.db_mod.propagate_skips(goal_id)
            self.assertEqual(count, 0)
...
class TestGetReadyTasksSkippedDeps(_DBTestBase):
    ...
    def test_no_deps_still_ready(self):
        ...
        async def _test():
            goal_id = await self._create_goal()
            t1 = await self._create_task(goal_id, "T1")
    ...
    def test_failed_deps_still_block(self):
        ...
        async def _test():
            goal_id = await self._create_goal()
            t1 = await self._create_task(goal_id, "T1", status="failed")
            t2 = await self._create_task(goal_id, "T2", depends_on=[t1])
    ...
    def test_pending_dep_still_blocks(self):
        ...
        async def _test():
            goal_id = await self._create_goal()
            t1 = await self._create_task(goal_id, "T1", status="pending")
            t2 = await self._create_task(goal_id, "T2", depends_on=[t1])
