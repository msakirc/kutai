...
class LLMDispatcher:
    ...
    async def _loaded_model_can_grade(self) -> bool:
        ...
        A task is gradeable by the loaded model if:
        - it was NOT generated by that model (no self-grading), AND
        - the loaded model is NOT in grade_excluded_models (not previously
          excluded due to parse failures on that task).
        """
        loaded = self._get_loaded_litellm_name()
        if not loaded:
            return False
