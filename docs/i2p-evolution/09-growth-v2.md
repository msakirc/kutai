...
- 2026-05-16 — review pass: wiring audit found T3's signal→backlog pipeline DEAD — `classify_signals` + `score_backlog` were orphaned executors (dispatchable, nothing scheduled them). Fixed: weekly `signal_classify_sweep` cron + classify→score continuation chain + corrected i2p 15.7/15.8 instruction text. Audit confirmed T1/T2/T4/T5 fully wired (verbs, crons, agents, telegram, reinforce loop, ab_result hook, recipe, anti-pattern tool all reachable). Tag `z9-complete-2026-05-15` re-pointed to the fix commit.
