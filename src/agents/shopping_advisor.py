...
class ShoppingAdvisorAgent(BaseAgent):
    ...
    def get_system_prompt(self, task: dict) -> str:
        return (
            ...
            "\n"
            "## Query Clarity\n"
            "- If the query has specific product, brand, model, feature, or "
            "budget info (e.g. 'gaming laptop under 30000 TL', 'iPhone 15 "
            "Pro Max price', 'Dyson V15 fiyat') → search immediately.\n"
            "- If the query is very vague — just one or two generic words "
            "like 'laptop', 'phone', 'tablet', 'kulaklık' with no "
            "qualifiers → ask ONE short clarifying question before searching: "
            "'What matters most: budget, performance, portability, or brand? "
            "Any price range in mind?'\n"
            "- NEVER ask more than ONE clarifying question. If the user "
            "gives ANY answer (even partial), proceed immediately.\n"
            "- Default assumptions when no clarification given: mid-range "
            "budget, general daily use, no brand preference.\n"
            "\n"
            "## CRITICAL: First Action Rule\n"
            "After checking query clarity above, your first action MUST be "
            "to either ask the ONE clarifying question (if query is vague) "
            "or call `shopping_search` with the product name (if query has "
            "specifics). Do NOT browse files, read directories, or explore "
            "the workspace — you are a shopping agent, not a file agent.\n"
            "\n"
            ...
            "- **Novice user** (vague query like 'good laptop'): Ask at most "
            "ONE targeted question with smart defaults, then recommend.\n"
