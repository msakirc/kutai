...
mr_roboto.run() and never reaches the LLM dispatch path. Result is reported to
...
@pytest.mark.asyncio
async def test_mechanical_executor_routes_to_mr_roboto():
    """executor='mechanical' → mr_roboto.run invoked, LLM path skipped, completed reported."""
    orch = _make_orch()
    task = {
        "id": 1,
        "mission_id": 2,
    ...
    with patch("src.core.orchestrator.mr_roboto.run", new_callable=AsyncMock) as mock_run, \
        ...
        mock_run.return_value = mr_roboto.Action(
...
async def test_mechanical_executor_failure_reports_failed():
    ...
    with patch("src.core.orchestrator.mr_roboto.run", new_callable=AsyncMock) as mock_run, \
         ...
         patch("src.core.orchestrator.inject_chain_context", new_callable=AsyncMock, return_value=task), \
         patch("src.core.orchestrator.release_task_locks", new_callable=AsyncMock):
        mock_run.return_value = mr_roboto.Action(status="failed", error="boom")
        await orch._dispatch(task)
...
async def test_no_executor_tag_still_takes_llm_path():
    """Regression guard: tasks without executor='mechanical' must not call mr_roboto.run."""
    orch = _make_orch()
    task = {
        "id": 5,
        "mission_id": 6,
    ...
    # that mr_roboto.run was NOT called.
