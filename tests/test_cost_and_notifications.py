...
class TestCostAccumulatesAcrossTasks(unittest.TestCase):
    ...
    def test_cost_accumulates_across_tasks(self, mock_db):
        ...
        async def _test():
            ...
            await write_blackboard(goal_id, "cost_tracking", {
            ...
            final = await read_blackboard(goal_id, "cost_tracking")
        ...
        async def _test():
            ...
            for threshold in thresholds:
                if prev < threshold <= total_cost:
                    await mock_send(
                        f"Goal #1 cost milestone: ${total_cost:.2f}\n"
...
class TestNoCostTrackingWithoutGoal(unittest.TestCase):
    """Tasks without a goal_id should not trigger cost tracking."""

    def test_no_cost_tracking_without_goal(self):
        # The cost tracking block in _handle_complete is gated by:
        #   if task.get("goal_id") and cost > 0:
        task_no_goal = {"id": 1, "title": "test", "context": "{}"}
        self.assertFalse(bool(task_no_goal.get("goal_id") and 0.05 > 0))

        task_zero_cost = {"id": 2, "title": "test", "goal_id": 5, "context": "{}"}
        cost = 0
        self.assertFalse(bool(task_zero_cost.get("goal_id") and cost > 0))
...
class TestStepFailureNotification(unittest.TestCase):
    ...
    def test_step_failure_notification(self):
        ...
        async def _test():
            ...
            task = {"id": 55, "title": "Implement auth module", "goal_id": 10}
...
class TestNoNotificationForNonWorkflowTasks(unittest.TestCase):
    ...
    def test_no_notification_for_non_workflow_tasks(self):
        ...
        async def _test():
            ...
            task_ctx = {}
            task = {"id": 77, "title": "Simple task", "goal_id": 5}
            ...
            if task_ctx2.get("is_workflow_step") and task.get("goal_id"):
