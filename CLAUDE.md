...
- **Telegram interface**: `src/app/telegram_bot.py` (TelegramInterface class, **~13,800 lines** — the largest god-file in the repo)
...
- **LLM dispatch**: `src/core/llm_dispatcher.py` — ask→load→call loop, delegates selection to Fatih Hoca. **622 LOC** (was 721; Phase 4 de-accretion 2026-06-05). No retry loop here — retry was deleted 2026-03-26 (d3e3c4fa); retries live in HaLLederiz Kadir (call-level) + Fatih Hoca selector feedback. Telemetry body moved to `src/telemetry/pick_recorder.py` (dispatcher keeps a thin `_record_pick` delegator for the test monkeypatch surface); loaded-model introspection → `src/models/introspection.py` (dead speed/thinking/name variants deleted); `_estimate_prompt_tokens` deleted (dead). Remaining non-loop bulk is the deprecated shopping-only `request()` shim + its 2 spec/response helpers (~190 LOC, **SP5-gated — deletes when shopping migrates to workflow steps**). See `docs/superpowers/specs/2026-06-05-dispatcher-deaccretion-design.md`.
...
- **NOT shims (fat logic mislabeled — see `docs/2026-05-31-root-debt-map.md`)**:
  - `src/core/router.py` (705) — live `get_kdv()` + `ModelCallFailed` ONLY. `select_model()`/`select_for_task()`/`check_cost_budget()` are **DEAD** (zero prod callers; live selection is `fatih_hoca/ranking.py::rank_candidates`). Router's scoring copy is stale + drifted — do NOT trust or edit it.
...
- **Dispatcher** (`execute()`): calls `fatih_hoca.select()`, loads via DaLLaMa, calls HaLLederiz Kadir. **No retry loop in the dispatcher** (deleted 2026-03-26); retry feedback lives in the selector + HaLLederiz Kadir. Owns message preparation (secret redaction, thinking adaptation) and timeout floors. 622 LOC after Phase 4 de-accretion (2026-06-05); remaining non-loop bulk is the SP5-gated `request()` shim.
...
| `src/app/telegram_bot.py` | All Telegram UI — commands, buttons, callbacks (~13,800 lines — needs split) |
| `src/core/orchestrator.py` | Pump loop: beckman.next_task + create_task dispatch. **793 LOC** (was 366 at Task 13; +427 re-added: workflow agent refresh, self-reflection bridge, yalayut discovery, mission resumption, founder sweep, heartbeat) |
| `src/core/llm_dispatcher.py` | ask→load→call loop (no retry), delegates to Fatih Hoca + HaLLederiz Kadir. **721 LOC**, not thin |
...
- **Known refactoring need**: `telegram_bot.py` (~13,800 lines) should be split into modules. Tree-wide modularization debt mapped in `docs/2026-05-31-root-debt-map.md` (see "src-wide" section) — top targets: telegram_bot 13.8k, `src/infra/db.py` 8.4k, workflow_engine half-extraction, shopping 25.9k sprawl.
