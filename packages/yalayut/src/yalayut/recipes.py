...
Schema (recipe_picks table) is registered via dabidabi.

Note: this is the disk-YAML recipe library (recipes/<name>/v1/recipe.yaml),
distinct from yalayut's yalayut_index ``shell_recipe`` discovery path. The two
recipe systems are co-located here but not yet converged (follow-up).
"""
...
logger = get_logger("yalayut.recipes")
