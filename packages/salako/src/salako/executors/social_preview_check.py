...
URLs from it, runs :func:`salako.parse_og_tags.parse_og_tags` on each, and
...
logger = get_logger("salako.social_preview_check")
...
async def _load_artifact(mission_id: int, name: str) -> dict | None:
    ...
    Mirrors the pattern used in salako.clarify so the same divergence cases
