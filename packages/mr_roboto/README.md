# Mr. Roboto — Mechanical Dispatcher

Turkish nickname for the steadfast worker who just does the grunt work without asking.

Mr. Roboto is the non-LLM executor sibling to the LLM dispatcher. It runs mechanical,
...
action = await mr_roboto.run(task)   # -> mr_roboto.Action(status, result, error)

# Direct helpers (backwards-compatible)
snap = await mr_roboto.snapshot_workspace(mission_id, task_id, workspace_path)
await mr_roboto.auto_commit(task, result)
```
...
```bash
pip install -e packages/mr_roboto
python -m pytest packages/mr_roboto/tests/
```

## TODO / notes

- `mr_roboto.workspace_snapshot` still imports from `src.infra.db` and `src.tools.*`.
