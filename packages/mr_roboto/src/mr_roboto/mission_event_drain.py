...
async def _drain_budget_alerts() -> int:
    ...
    Table may not exist yet (T2A not merged) — swallow OperationalError.
    """
    from src.infra.db import get_db
    ...
    try:
        cur = await db.execute(
            "SELECT id, mission_id, threshold_pct, total_cost, ceiling "
            "FROM mission_budget_alerts WHERE telegram_event_id IS NULL"
        )
        rows = await cur.fetchall()
    except Exception as e:  # noqa: BLE001
        logger.debug(
            "drain_budget_alerts: mission_budget_alerts unavailable (T2A?), skip",
            error=str(e),
        )
        return 0
    ...
    posted = 0
    for row in rows:
        aid, mission_id, threshold, total, ceiling = (
            row[0], row[1], row[2], row[3], row[4],
        )
        try:
            event_id = await post_event(
            ...
            await db.execute(
                "UPDATE mission_budget_alerts SET telegram_event_id = ? "
                "WHERE id = ?",
                (int(event_id), int(aid)),
            )
            await db.commit()
            posted += 1
        except Exception as e:  # noqa: BLE001
