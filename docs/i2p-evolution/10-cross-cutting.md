...
| Sandbox escape | **HIGH** | `src/tools/shell.py` mode=`local` falls back to host subprocess at `_run_quiet` (lines 125-133) with no audit-log enrichment (no `sandbox_mode` field on log lines); `BLOCKED_PATTERNS` (lines 60-77) blocks `rm -rf /` literal but not `subprocess.run(["dd", "if=/dev/sda", ...])` constructed via tool calls |
