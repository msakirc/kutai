...
async def test_drain_budget_alert_when_table_present(tmp_path, monkeypatch):
    ...
    # Simulate T2A's mission_budget_alerts shape.
    await db.execute(
        "CREATE TABLE mission_budget_alerts ("
    ...
    await db.execute(
        ...
        "(mission_id, threshold_pct, total_cost, ceiling) VALUES (?, ?, ?, ?)",
        (mid, 80, 1.23, 1.50),
    )
    await db.commit()
    ...
    cur = await db.execute(
        "SELECT telegram_event_id FROM mission_budget_alerts"
    )
    row = await cur.fetchone()
    assert row[0] is not None
    cur = await db.execute(
        "SELECT kind FROM mission_events WHERE id = ?", (row[0],),
    )
