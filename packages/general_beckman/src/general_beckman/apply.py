...
def _posthook_agent_and_payload(
    ...
    if a.kind == "test_run":
        # Mechanical post-hook: mr_roboto picks the right runner (pytest /
        # jest / vitest) from the target file extensions.  The step's
        # ``produces`` list is passed as ``target_files``; optional
        # ``stack_hint`` in the source context steers jest vs vitest.
        produces = list(source_ctx.get("produces") or [])
        return ("mechanical", {
            "source_task_id": a.source_task_id,
