...
- **Phase 2c machinery (foundation, not balanced yet, 2026-04-19)**: `packages/fatih_hoca/src/fatih_hoca/pools.py` classifies every candidate into `local` / `time_bucketed` / `per_call`. `ranking.py::_apply_urgency_layer` adds a bounded (+25%) composite bonus for candidates with computed urgency when they pass a `cap_score ≥ 0.85 × top_cap` gate. `model_pick_log` now stores `pool` + `urgency` per row and `cap_score` per candidate in `candidates_json`. Simulator fixture uses `idle_seconds=300` to exercise the local path. **The gate currently over-blocks**: when `claude-sonnet` is a candidate, the gate threshold becomes unreachable for most locals and even groq-70B, so urgency rarely fires. Phase 2d replaces urgency+gate with a unified "utilization fit" equation on a stateful per-task simulator. Until then, treat Phase 2c as telemetry + schema + classification infrastructure, not as a closed-loop balance mechanism.
